...
SELECT  t2.order_id,
        ...
        t2.customer_email 
FROM source2.craft_market_masters_products AS t1 
     INNER JOIN source2.craft_market_orders_customers AS t2 
...
SELECT  t1.order_id,
        ...
        t3.customer_email
FROM source3.craft_market_orders AS t1
     INNER JOIN source3.craft_market_craftsmans AS t2 
     ON t1.craftsman_id = t2.craftsman_id 
     INNER JOIN source3.craft_market_customers AS t3 
...
USING (SELECT DISTINCT craftsman_name,
                       craftsman_address,
                       craftsman_birthday,
                       craftsman_email
       FROM tmp_sources
...
USING (SELECT DISTINCT product_name,
                       product_description,
                       product_type,
                       product_price
       FROM tmp_sources
...
USING (SELECT DISTINCT customer_name,
                       customer_address,
                       customer_birthday,
                       customer_email
       FROM tmp_sources
